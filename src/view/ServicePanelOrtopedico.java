...
import dominio.ServiziPanel;

@SuppressWarnings("serial")
public class ServicePanelOrtopedico extends JFrame {
    private ServiziDAOimp serviziDAO = new ServiziDAOimp("Ortopedico");
    ...
    private void loadServicesByCodiceFiscale(String codiceFiscale) {
        try {
			List<ServiziPanel> servizio = serviziDAO.searchByCodiceFiscale(codiceFiscale);
    ...
    private void saveAllRowsToDatabase() {
        for (int i = 0; i < tableModel.getRowCount(); i++) {
            ...
            if (validateFields(prezzo, descrizione, trattamento, codiceFiscale)) {
				...
				ServiziPanel servizi = new ServiziPanel(prezzo, descrizione, trattamento, codiceFiscale, Tipo_di_utente);
                try {
    ...
    public static void main(String[] args) {
        ...
        frame.add(new ServicePanelOrtopedico()); // Sostituisci con un'email valida
